...
class MongoDB(object){
    ...
    GENETIC_DB_NAME='genetic_db'
    NEURAL_DB_NAME='neural_db'
    DUMMY_FOLDER='tmp/core/DummyMongo/'
    ...
    def startMongoDatabases(self){
        ...
        if MongoDB.GENETIC_DB_NAME not in dblist{
            self.logger.warn('Database {} does not exists, creating it...'.format(MongoDB.GENETIC_DB_NAME))
        }
        self.genetic_db = self.client[MongoDB.GENETIC_DB_NAME]
        if MongoDB.NEURAL_DB_NAME not in dblist{
            self.logger.warn('Database {} does not exists, creating it...'.format(MongoDB.NEURAL_DB_NAME))
        }
        self.neural_db = self.client[MongoDB.NEURAL_DB_NAME]
    }
    ...
    def getGeneticDB(self){
        return self.genetic_db
    }

    def getNeuralDB(self){
    ...
    def startQueue(self,id=0){ 
        consumer_id='core_{}-{}'.format(socket.gethostname(),id)
    ...
    def findOneOnDBFromIndex(self,db,collection,index_field,index_value,wait_unlock=True){
        if index_field=='_id' and type(index_value) is str{
            index_value=self.getObjectId(index_value)
        }
